...
export default function ProductosComercioClient({
  businessId,
  initialRelations,
  initialBusinessName,
}: Props) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `/api/affiliatedbusinessxproduct/${businessId}/get`,
        { cache: "no-store" }
      );
    ...
    <div className="min-h-screen px-4 md:px-8 lg:px-12 py-6 space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Button
          variant="secondary"
          onClick={() =>
            router.push(`/admin/consultas/comercios/${businessId}`)
          }
        >
          Volver
        </Button>
        ...
        <CardContent className="space-y-4">
          ...
          {!loading && relations.length === 0 && (
            <p className="text-sm text-muted-foreground">
              Este comercio no tiene productos registrados.
            </p>
          )}
