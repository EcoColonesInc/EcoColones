...
export default function ComerciosClient({ initialBusinesses, initialTypes, initialTopProducts }: ComerciosClientProps) {
  const router = useRouter();
  const [businesses, setBusinesses] = useState<AffiliatedBusiness[]>(initialBusinesses as AffiliatedBusiness[]);
  const [types, setTypes] = useState<BusinessType[]>(initialTypes as BusinessType[]);
  const [topProducts, setTopProducts] = useState<TopProduct[]>(initialTopProducts as TopProduct[]);
  ...
  async function refreshBusinesses() {
    try {
      setLoading(true);
      const res = await fetch("/api/affiliatedbusiness/get", { cache: "no-store" });
  ...
  async function refreshTopProducts() {
    try {
      ...
      setTopError(null);
        ...
        setTopError(err instanceof Error ? err.message : "Error cargando top de productos");
    } finally {
      setTopLoading(false);
    }
  }
  ...
  async function fullRefresh() {
    await Promise.all([refreshBusinesses(), refreshTypes(), refreshTopProducts()]);
  }
  ...
  return (
    <div className="min-h-screen px-4 md:px-6 lg:px-8 py-6 space-y-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={fullRefresh} disabled={loading || topLoading}>
                    ...
                    : filtered.map((b) => (
                        <TableRow key={b.affiliated_business_id}>
                          ...
                          <TableCell className="text-center">
                            <Button size="sm" variant="default" onClick={() => router.push(`/admin/consultas/comercios/${b.affiliated_business_id}`)}>Seleccionar</Button>
                          </TableCell>
                        </TableRow>
                      ))}
                </TableBody>
        ...
        <div className="flex flex-col gap-6">
          ...
          <FiltersPanel>
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2">
                <input className="border rounded px-2 py-1 flex-1" placeholder="Ingresa el nombre del gerente" value={fManager} onChange={(e) => setFManager(e.target.value)} />
                {fManager && <FilterTag label="Nombre del gerente" onClear={() => setFManager("")} />}
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <label className="font-medium">Tipo de comercio</label>
              <select className="border rounded px-2 py-1" value={fType} onChange={(e) => setFType(e.target.value)}>
                <option value="">Todos</option>
                {types.map((t) => (
                  <option key={String(t.business_type_id)} value={t.name}>{t.name}</option>
                ))}
              </select>
            </div>
            <button onClick={() => { setFName(""); setFManager(""); setFType(""); }} className="border w-full rounded px-2 py-1 bg-muted">Limpiar filtros</button>
          </FiltersPanel>
        </div>
      </div>
    </div>
