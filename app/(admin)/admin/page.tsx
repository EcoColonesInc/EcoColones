...
export default function AdminHomePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const info = Array.isArray(data) ? (data[0] as Record<string, unknown> | undefined) : (data as Record<string, unknown> | undefined);
  ...
  return (
    <div className="flex min-h-[calc(100vh-56px)] flex-col">{/* 56px ≈ altura navbar */}
      <section className="flex-1 grid grid-cols-1 lg:grid-cols-2 items-center gap-6 px-6 md:px-12 lg:px-16 py-10">
        {/* Logo grande */}
        <div className="flex justify-center lg:justify-start">
          <Image src="/logo.png" alt="EcoColones" width={420} height={420} className="w-[20rem] h-[20rem] sm:w-[22rem] sm:h-[22rem] lg:w-[26rem] lg:h-[26rem] rounded-full" />
        </div>
        {/* Texto de bienvenida */}
      ...
      <footer className="px-6 md:px-12 lg:px-16 pb-6">
        <div className="flex items-center justify-between text-sm text-foreground/80">
          <Link href="#" className="hover:underline">Políticas y Privacidad</Link>
          <Link href="#" className="hover:underline">Términos de servicio</Link>
        </div>

        <div className="mt-10 flex flex-col items-center gap-2">
          <div className="flex items-center gap-4 text-foreground/80">
            <Link href="#" aria-label="Instagram" className="hover:text-foreground"><Instagram className="h-4 w-4" /></Link>
            <Link href="#" aria-label="Twitter" className="hover:text-foreground"><Twitter className="h-4 w-4" /></Link>
            <Link href="#" aria-label="Facebook" className="hover:text-foreground"><Facebook className="h-4 w-4" /></Link>
          </div>
          <p className="text-xs text-foreground/70 mt-2">©2025 EcoColones Inc. Todos los derechos reservados</p>
        </div>
      </footer>
    </div>
  );
