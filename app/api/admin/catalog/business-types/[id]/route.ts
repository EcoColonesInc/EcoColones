...
export async function DELETE(
  ...
  try {
    ...
    const businessTypeId = parseInt(id);
    ...
    const { count } = await supabase.from("affiliatedbusiness").select("*", { count: "exact", head: true }).eq("business_type_id", businessTypeId);
    if (count && count > 0) {
      return NextResponse.json({ error: `No se puede eliminar. Hay ${count} negocio(s) usando este tipo.` }, { status: 400 });
    }
