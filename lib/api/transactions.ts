...
export async function getAllCollectionCentersTransactions() {
  ...
  const { data, error } = await supabase
    ...
    .select('person_id(user_name, first_name, last_name), collection_center_id(name), material_id(name, equivalent_points), total_points, material_amount, transaction_code, created_at')
